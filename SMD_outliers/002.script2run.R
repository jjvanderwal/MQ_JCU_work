...
for (coi in Algs) { cat('\t',coi,'\n')
	tdata = out[,coi]
	if (all(is.na(tdata)) & !coi %in% Algs.geo ) {
		if (is.null(sum.out)) {
			sum.out = NULL
		} else {
			sum.out = as.data.frame(rbind(sum.out,c(spp,coi,rep(NA,ncol(sum.out)-2))),stringsAsFactors=FALSE)
		}
		...
		if (coi %in% Algs.biomod) {
			...
			tout = cbind(data.frame(spp=spp,alg=coi,raw.threshold=tt$threshold.max.sensitivity.specificity[1]*1000,as.data.frame(tt)[1,]),accuracy(out$pa,tdata,tt$threshold.max.sensitivity.specificity[1]))
		} else if (coi %in% c("brt","mahal")) {
			tdata = (out[,coi] + abs(min(out[,coi],na.rm=TRUE)))/diff(range(out[,coi],na.rm=TRUE))
			tt = optim.thresh(out$pa,tdata,101); tt = as.data.frame(tt)[1,]; colnames(tt) = paste('threshold.',colnames(tt),sep='') #kappa.fun(out$pa,tdata)
			tout = cbind(data.frame(spp=spp,alg=coi,raw.threshold=tt$threshold.max.sensitivity.specificity[1]*diff(range(out[,coi],na.rm=TRUE))-abs(min(out[,coi],na.rm=TRUE)),as.data.frame(tt)[1,]),accuracy(out$pa,tdata,tt$threshold.max.sensitivity.specificity[1]))
		} else {
			tt = optim.thresh(out$pa,tdata,101); tt = as.data.frame(tt)[1,]; colnames(tt) = paste('threshold.',colnames(tt),sep='') #kappa.fun(out$pa,tdata)
			tout = cbind(data.frame(spp=spp,alg=coi,raw.threshold=tt$threshold.max.sensitivity.specificity[1],as.data.frame(tt)[1,]),accuracy(out$pa,tdata,tt$threshold.max.sensitivity.specificity[1]))
		}
		...
		tout = cbind(tout,tt2)
		sum.out = as.data.frame(rbind(sum.out,tout),stringsAsFactors=FALSE) #record the info out
	}
}
