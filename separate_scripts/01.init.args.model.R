...
wd = "/home/jc140298/ibccvl/ABT/" #define the working directory - where to put the outputs
species = "ABT"	#define the species of interest
occur.data = "/home/jc140298/ibccvl/ABT/occur.csv" #define the lon/lat of the observation records -- 2 column matrix of longitude and latitude
bkgd.data = "/home/jc140298/ibccvl/ABT/bkgd.csv" #define the the lon/lat of the background / psuedo absence points to use -- 2 column matrix of longitude and latitude
...
biomod.VarImport = 0 #number of resampling of each explanatory variable to measure the relative importance of each variable for each selected model
...
model.gbm = TRUE #boolean to run generalized boosting model algorithm
if (model.gbm) {
...
model.ann = TRUE #boolean to run artificial neural network algorithm
if (model.ann) {
...
model.mars = TRUE #boolean to run multiple adaptive regression splines algorithm
if (model.mars) {
...
save.image(paste(wd, "/01.init.args.model.", species, ".RData", sep=""))